function setupHTML() {
	...
	let table = ""
	let table2 = ""
	for (let x = 0; x < TABS[0].length; x++) {
		table += `<div>
			<button onclick="TABS.choose(${x}); player.stab = 0;" class="btn_tab" id="tab${x}"><div>${TABS[0][x].id}</div></button>
		</div>`
		if (TABS[1][x]) {
			let a = `<div id="stabs${x}" class="table_center stab_btn">`
			for (let y = 0; y < TABS[1][x].length; y++) {
				a += `<div style="width: 160px">
					<button onclick="TABS.choose(${y}, true); player.stab = 0;" class="btn_tab" id="stab${x}_${y}">${TABS[1][x][y].id}</button>
				</div>`
			}
			a += `</div>`
			table2 += a
...
function updateTabsHTML() {
	...
	for (let x = 0; x < TABS[1].length; x++) {
		...
		if (TABS[1][x]) {
			...
			if (x == tmp.tab) for (let y = 0; y < TABS[1][x].length; y++)  {
				let stab = TABS[1][x][y]
				...
				if (tmp.el["stab_frame"+x+"_"+y]) tmp.el["stab_frame"+x+"_"+y].setDisplay(player.stab == x)
			}
		}
	}
}
